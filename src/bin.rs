...
enum Commands{
    New{
        ...
        name: String,
        #[arg(short,long,help="(Optional) Local path to the vault. Use local://<path> format for local files")]
        path: Option<String>,
    },
    List,
    Unlock{
...
fn main(){
    ...
    match cli.command {
        Commands::New{name, path} => {
            ...
            let repeat_password: String = rpassword::prompt_password("Repeat vault password: ").expect("Failed to read password");
            if password != repeat_password {
                eprintln!("Passwords do not match. Please try again.");
                return;
            }
            println!("Adding vault '{}' at path '{}'", name, path);
            manager.add_vault(name, path, HashMap::new(), password).expect("Failed to add vault");
        },
        Commands::List => {
        ...
        Commands::Unlock{name} => {
            ...
            if manager.unlock_vault(&name, &password).is_ok() {
                ...
                eprintln!("Failed to unlock vault '{}'. Please check the name and password.", name);
            }
        },
