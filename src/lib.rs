...
impl Vault{
    pub fn new(storage:Box<dyn StorageBackend>,master_password:&str)->Result<Self,VaultError>{
    ...
    fn save_metadata(&self)->Result<(),VaultError>{
        ...
        self.storage.store_blob(METADATA_FILENAME, metadata_json.as_bytes())
    }
...
impl LocalStorageBackend{
 ...
 pub fn new(root_path: PathBuf) -> Result<Self, VaultError> {
    if root_path.exists() {
        if root_path.is_dir() {
            let is_empty = root_path.read_dir()
